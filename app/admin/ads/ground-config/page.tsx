...
export default function GroundBannerConfigPage() {
    ...
    useEffect(() => {
        fetchGroundBannerConfig()
    }, [])

    const fetchGroundBannerConfig = async () => {
        try {
            // Get ground_banner placement
            const placementsRes = await fetch('/api/ads/placements')
            const placementsData = await placementsRes.json()
            const groundPlacement = placementsData.placements?.find(
                (p: any) => p.placement_key === 'ground_banner'
            )

            if (groundPlacement) {
                setPlacementId(groundPlacement.id)

                // Get existing contents
                const contentsRes = await fetch(`/api/ads/contents?placement_id=${groundPlacement.id}`)
    ...
    const handleSave = async () => {
        ...
        console.log('[GroundBannerConfig] Saving logos:', logos)
        console.log('[GroundBannerConfig] Placement ID:', placementId)
        ...
        try {
            ...
            console.log('[GroundBannerConfig] Deleting existing contents:', existingContents.length)
            ...
            for (let i = 0; i < logos.length; i++) {
                const logo = logos[i]
                console.log(`[GroundBannerConfig] Processing logo ${i}:`, logo)
                ...
                if (logo.sponsor_name && logo.logo_url) {
                    ...
                    console.log(`[GroundBannerConfig] Saving logo ${i}:`, payload)
                    ...
                    if (res.ok) {
                        savedCount++
                        console.log(`[GroundBannerConfig] Logo ${i} saved successfully`)
                    } else {
                        console.error(`[GroundBannerConfig] Failed to save logo ${i}:`, await res.text())
                    }
                } else {
                    console.log(`[GroundBannerConfig] Skipping logo ${i} - missing name or URL`)
                }
            }

            console.log(`[GroundBannerConfig] Total saved: ${savedCount} logos`)
            alert(`✅ Lưu thành công ${savedCount} logos!`)
            fetchGroundBannerConfig()
        } catch (error) {
            console.error('[GroundBannerConfig] Save error:', error)
            alert('❌ Lưu thất bại')
        } finally {
            setSaving(false)
        }
                ...
                <div className="flex items-center justify-between mb-8">
                    <div>
                        ...
                        <h1 className="text-3xl font-bold text-white mb-2">
                            🎯 Cấu hình Ground Banner Ads
                        </h1>
                        <p className="text-white/60">Thiết lập 4 logos nhà tài trợ hiển thị dưới game canvas</p>
                    </div>
                </div>
                ...
                {loading ? (
                    ...
                    <>
                        ...
                        <div className="bg-white/10 backdrop-blur-sm rounded-xl p-6 border border-white/20 mb-6">
                            <h3 className="text-white font-bold mb-4">👁️ Preview (như trên trang Game)</h3>
                            <div className="bg-gradient-to-br from-green-500/10 via-emerald-500/10 to-green-500/10 rounded-2xl p-3 border border-green-500/20">
                                    ...
                                    >
                                        ...
                                        <div className="mb-3">
                                            ...
                                            <input
                                                ...
                                                placeholder={`VD: Nhà tài trợ ${index + 1}`}
                            ...
                            <div className="flex items-start gap-3">
                                ...
                                <div>
                                    ...
                                    <ul className="text-white/70 text-sm space-y-1">
                                        ...
                                        <li>• Hiển thị dưới game canvas khi không đang chơi</li>
