...
export default function VoucherRedemptionConfigPage() {
    ...
    const fetchVoucherConfig = async () => {
        try {
            // Get voucher_redemption placement
            const placementsRes = await fetch('/api/ads/placements')
            const placementsData = await placementsRes.json()
            const voucherPlacement = placementsData.placements?.find(
                (p: any) => p.placement_key === 'voucher_redemption'
            )

            if (voucherPlacement) {
                setPlacementId(voucherPlacement.id)

                // Get existing contents
                const contentsRes = await fetch(`/api/ads/contents?placement_id=${voucherPlacement.id}`)
    ...
    const handleSave = async () => {
        ...
        console.log('[VoucherConfig] Saving logos:', logos)
        console.log('[VoucherConfig] Placement ID:', placementId)
        ...
        try {
            ...
            console.log('[VoucherConfig] Deleting existing contents:', existingContents.length)
            ...
            for (let i = 0; i < logos.length; i++) {
                const logo = logos[i]
                console.log(`[VoucherConfig] Processing logo ${i}:`, logo)
                ...
                if (logo.sponsor_name && logo.logo_url) {
                    ...
                    console.log(`[VoucherConfig] Saving logo ${i}:`, payload)
                    ...
                    if (res.ok) {
                        savedCount++
                        console.log(`[VoucherConfig] Logo ${i} saved successfully`)
                    } else {
                        console.error(`[VoucherConfig] Failed to save logo ${i}:`, await res.text())
                    }
                } else {
                    console.log(`[VoucherConfig] Skipping logo ${i} - missing name or URL`)
                }
            }

            console.log(`[VoucherConfig] Total saved: ${savedCount} logos`)
            alert(`✅ Lưu thành công ${savedCount} logos!`)
            fetchVoucherConfig()
        } catch (error) {
            console.error('[VoucherConfig] Save error:', error)
            alert('❌ Lưu thất bại')
        } finally {
            setSaving(false)
        }
                ...
                <div className="flex items-center justify-between mb-8">
                    <div>
                        ...
                        <h1 className="text-3xl font-bold text-white mb-2">
                            🎁 Cấu hình Voucher Redemption Ads
                        </h1>
                        <p className="text-white/60">Thiết lập 4 logos nhà tài trợ voucher</p>
                    </div>
                </div>
                ...
                {loading ? (
                    ...
                    <>
                        ...
                        <div className="bg-white/10 backdrop-blur-sm rounded-xl p-6 border border-white/20 mb-6">
                            <h3 className="text-white font-bold mb-4">👁️ Preview (như trên trang Voucher)</h3>
                            <div className="bg-gradient-to-br from-purple-500/10 via-pink-500/10 to-purple-500/10 rounded-2xl p-3 border border-purple-500/20">
                            ...
                            <div className="flex items-start gap-3">
                                ...
                                <div>
                                    ...
                                    <ul className="text-white/70 text-sm space-y-1">
                                        <li>• Điền tên và upload logo cho tối đa 4 nhà tài trợ voucher</li>
